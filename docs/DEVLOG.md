...
## 2026-08-01 — 改採 Apache 2.0

**BSL 1.1 換成 Apache License 2.0。** 這是不可逆的決定，所以把推理過程完整記下來。

### 起因是一個看起來無關的問題：簽章要多少錢

未簽章的安裝檔會讓 Windows SmartScreen 跳警告，而憑證是發給**經過身分驗證的法律實體**的，
一年數百美元。查下去才發現生態系其實對開源專案很便宜：

| | 價格 | 條件 |
|---|---|---|
| SignPath Foundation | **免費** | **OSI 認可的授權**，不得商業雙授權 |
| Certum 開源開發者憑證 | 首年 €69，續約 €29/年 | 個人，需證明參與開源專案 |
| Microsoft Store（MSIX） | **$19 一次性** | 微軟代簽；但與 Qt LGPL 的重新連結要求有張力 |
| 一般 OV / EV | $200–700/年 | 企業 |

**貴的那一檔是賣給公司的。** 價格結構本來就是「開源免費、商業付費」，
而 LumenPDF 剛好卡在中間——source-available 但不是 OSI 開源，兩邊的好處都拿不到。

### 但真正的問題不是省 19 塊美金

真正的問題是：**BSL 保護的商業收入是不是真的存在？**

結論是不存在。BSL 的商業條款要生效，得有公司主動來買授權，
而企業採購要的是支援合約、SLA、責任歸屬——那些東西背後要有一間公司。
個人的 source-available 專案，採購流程根本不會受理。
所以 BSL 現實上的效果不是「公司付錢」，是「**公司乾脆不用**」。

唯一可能值錢的是**塗黑功能**（法務電子開示、政府文件公開確實會付費），
但那些單位買的是責任承擔，同樣需要一間公司在後面。

### BSL 正在收的三筆費用

- **Linux 發行版永遠收錄不了。** Debian / Fedora / Arch 的官方倉庫都要求
  OSI / DFSG 認可的授權。而 macOS 和 Linux 就在規劃裡——這兩件事在打架。
- **免費簽章拿不到。** SignPath 明確要求 OSI 授權。
- **貢獻者的門檻。** 現在最缺的兩樣東西——更多介面語言、macOS/Linux 移植——
  正好是社群最擅長貢獻的。授權是門檻。

### 時機：現在改最乾淨

重新授權需要擁有全部著作權。目前沒有任何外部貢獻者，一個 commit 就改完；
等到有五個人送過 PR，就得一個一個去要同意，有人不回信那段程式碼就卡住。

**這一點推翻了「等有證據再決定」的預設。** 一般來說不可逆的決定應該延後，
但這裡延後會讓執行成本上升，而且 BSL 本來就排定 2030-07-30 自動轉 Apache 2.0——
等於只是把已經決定的事情提前四年，換四年的實際好處。

### 順帶釐清的兩件事

**商標和授權是兩回事。** Apache 2.0 第 6 條明確不授予商標權。
別人可以 fork、修改、拿去賣，但**不能叫 LumenPDF**。
「怕被人拿去賣」從來就不是留 BSL 的理由。

**安裝程式不再顯示授權頁。** BSL 底下有東西是使用者真的需要同意的；
Apache 2.0 什麼都給、什麼都不要求，再放一個 click-through 只是儀式。
`LICENSE.txt` 和 `NOTICE.txt` 照樣放進安裝目錄，
`LICENSES\` 帶第三方授權文字——那才是義務真正要求的東西。

**新增 `NOTICE`**：Apache 2.0 第 4(d) 條規定，作品若附帶 NOTICE，
再散布時必須一併包含。所以打包腳本現在會把它放進去。

### 資金模式

不是商業授權，是**捐贈**。而捐贈要的是觸及率，
BSL 唯一的作用就是製造摩擦——兩者直接矛盾。

順帶一提，換成捐贈模式之後，**簽章從面子問題變成直接影響收入**：
每一個在 SmartScreen 警告前放棄的人，都是一個永遠不會捐款的人。
而 Apache 2.0 讓簽章免費。這兩件事現在是同一個決定。

**不會在 app 裡放捐贈提示。** 一個標榜「簡潔、克制」的閱讀器跳出討錢視窗，
會毀掉整個定位。
...
> **這個決定在 2026-08-01 被推翻了，改採 Apache 2.0。** 下面是當時的推理，保留原樣。
> 推翻的理由見本檔最上方那則——簡短版：它保護的商業收入不存在，
> 而代價是 Linux 發行版收錄不了、免費簽章拿不到、貢獻者進不來。
